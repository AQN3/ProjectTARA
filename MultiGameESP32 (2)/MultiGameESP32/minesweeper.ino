...
void runMinesweeper(bool isMultiplayer) {
    ...
    vga.clear(0);
    resetGame();  // Ensure the game initializes correctly
    ...
    while (gameRunning) {  
        ...
        if (prevX != selectedX || prevY != selectedY) {
            drawTile(prevX, prevY);  // Redraw previous tile to remove old highlight
            updateSelection(selectedX, selectedY);  // **Ensure cursor highlight updates properly**
            drawGrid();  
        }
