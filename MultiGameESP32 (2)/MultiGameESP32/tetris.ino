...
void drawBorder() {
   ...
   vga.line(120, 4 * squareY + 2, 120 + squareX * (myColumns - 1) - 2, 4 * squareY + 2, vga.RGB(255, 255, 255));
   vga.setTextColor(vga.RGB(255, 255, 0), vga.RGB(0, 0, 0));  // Yellow text
   vga.setCursor(128, 10);  // Adjust position (x, y)
   vga.print("TETRIS");
...
void myColor(int color, int scheme = 3) { 
    // Use `scheme` to select different color palettes 
    switch (scheme) {
        case 2: // Standard Colors
            if (color == 1){colA = 0; colB = 0; colC = 0;} // Black
            if (color == 2){colA = 1; colB = 0; colC = 0;} // Red
            if (color == 3){colA = 0; colB = 1; colC = 0;} // Green
            if (color == 4){colA = 0; colB = 0; colC = 1;} // Blue
            if (color == 5){colA = 1; colB = 1; colC = 0;} // Yellow
            if (color == 6){colA = 1; colB = 0; colC = 1;} // Magenta
            if (color == 7){colA = 0; colB = 1; colC = 1;} // Cyan
            if (color == 0){colA = 1; colB = 1; colC = 1;} // White
            break;
...
void mySquare(int x0, int y0, int color){
   ...
   vga.fillRect(x0 + 1, y0 + 1, squareX - 3, squareY - 2, vga.RGB(colA*255, colB*255, colC*255)); 
    if (color != 0) { myColor(color); }
    if (color == 7) { myColor(color); }
