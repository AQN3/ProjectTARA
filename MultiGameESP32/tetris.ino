...
void runTetris(bool isMultiplayer) {
    ...
    while (true) {  // Main game loop
        ...
        if (digitalRead(button5) == HIGH) {  // **Exit Game (Button 5)**
            vga.clear(0);  // Clear screen before exiting
            delay(100);  // Prevent button hold issues
        ...
        myTime++;
        if (myTime % fast > fast - 2 || button_4 == 1) {  // Fast Drop (Button 4)
            if (fast < 3) {fast = 2;}
            y = y + 2;
            delBlock();
            replaceBlock();
